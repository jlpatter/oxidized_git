...
use git_manager::{GraphOps, GitManager};
...
fn emit_update_all(git_manager: &mut MutexGuard<GitManager>, commit_ops: GraphOps, main_window: &Window<Wry>) {
                        ...
                        if *just_got_repo {
                            *just_got_repo = false;
                            ...
                            thread::spawn(move || {
                                ...
                                emit_update_all(&mut git_manager, GraphOps::Both, &main_window_c_c);
                            });
                        }
                    }
                },
            ...
            match event.menu_item_id() {
                ...
                "init" => {
                    ...
                    match result {
                        Ok(did_init) => {
                            if did_init {
                                emit_update_all(&mut git_manager, GraphOps::DifferentRepo, &main_window_c);
                                let mut just_got_repo = just_got_repo_arc_c.lock().unwrap();
                                *just_got_repo = true;
                            } else {
            ...
            thread::spawn(move || {
                match event.payload() {
                    Some(s) => {
                        ...
                        match result {
                            Ok(()) => emit_update_all(&mut git_manager, GraphOps::DeletedOnly, &main_window_c_c),
                            Err(e) => handle_error(e, &main_window_c_c),
                        };
                    },
                    None => main_window_c_c.emit_all("error", "Failed to receive payload from front-end").unwrap(),
            ...
            thread::spawn(move || {
                match event.payload() {
                    Some(s) => {
                        ...
                        match result {
                            Ok(()) => emit_update_all(&mut git_manager, GraphOps::RefChange, &main_window_c_c),
                            Err(e) => handle_error(e, &main_window_c_c),
                        };
                    },
                    None => main_window_c_c.emit_all("error", "Failed to receive payload from front-end").unwrap(),
                ...
                Some(s) => {
                    match config_manager::save_preferences(s) {
                        Ok(()) => {
                            ...
                            thread::spawn(move || {
                                let mut git_manager = git_manager_arc_c_c.lock().unwrap();
                                emit_update_all(&mut git_manager, GraphOps::ConfigChange, &main_window_c_c);
                            });
                        },
                        Err(e) => handle_error(e, &main_window_c),
                    };
